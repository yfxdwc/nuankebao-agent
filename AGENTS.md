...
**项目**: 暖客宝(NuankeBao)· 大健康行业**客户管理系统** —— **核心 = 客户管理** (记录 / 管理 / 分析) + **AI 助手给跟进指引** → **提高付费率 + 加盟率** (完整四要素见 CHARTER §1.4 + ADR-0018, 2026-09-23 主人核实)
...
**核心交互** (对齐 CHARTER §1.4 三大动作 + 行动输出):
- **记录**: 养生记录结构化表单 (部位 / 状态 / 用料 / 效果) — 为"分析"提供原料
- **管理**: 客户画像 + 跟进任务 — 一个客户 = 她的所有历史 + 待办
- **跟进指引** (AI 助手核心产出): 跟进建议 + 话术生成 (Phase 2); 不为炫技, 为"在对的时机做对的事" → 服务付费 / 加盟转化
...
- ✅ **客户 ↔ 账号 显式绑定 (ADR-0016 D10)**: 手工客户注册 app 后手机号可能对不上 (系统自动认领不到)
  → 客户详情页「app 身份」卡填她的**邀请码**绑定; 若系统已按她的号自动建了空档案 → **接管它**
  (删空档案 + 手机号对齐成真号)。口径唯一真相源 = `src/lib/db/queries/customer.ts::bindCustomerAccount`;
