...
```
「我的」页 (/profile, 主页面, 高频访问, 管理都从这里进入)
  └─ 区块 1‒4 保留: 个人资料 / 我的加盟身份 / 数据概览 / 设置 (字号快速 + 更多设置入口)
  └─ 区块 5: 账号与安全
  └─ 版本升级入口 (最底, 轻量: 本机版本 + 检查更新; 2026-09-25)

「设置」页 (/profile/settings, 二级页, 路由 push; 保留返回栈)
  └─ 区块 1: 显示与存储 (字号 4 档 chips + 清理图片缓存)
  └─ 区块 2: 主题配色 (换肤; ThemePickerCard)
  └─ 区块 3: 提醒 (本地通知跟进)
  └─ 区块 4: 关于与帮助 (使用帮助 / 网络自检 / 管理员工具[仅 admin] / debug 服务地址)
  └─ 区块 5: 退出登录 (最底, 危险操作二次确认; 2026-09-25 从「我的」页迁入)
```
...
| **高频留主页面, 低频下沉二级** | 个人/加盟/数据/账号 4 块在「我的」; 字号以外的系统设置 (主题/提醒/关于/退出登录/调试) 进「设置」; 字号两处都给 (中年刚需) |
...
| 版本入口 | 本机版本 (vX.Y.Z (N)) + 点一下 = 检查更新弹层 (轻量, 非大红按钮) | `package_info_plus` + `/api/app-version` |
...
| 4 关于与帮助 | 使用帮助 / 数据安全 (跳 `/profile/about`) + 网络自检 + 管理员工具 (仅 `role='admin'` 可见, 跳 `/profile/admin`) + debug 服务地址 | `/api/health` + `/api/app-version` |
| 5 退出登录 | 危险操作, 二次确认 (数据不受影响); 2026-09-25 从「我的」页迁入 (原版大红按钮, 设置页底部保留) | — |
