...
>
> **主人拍板记录 (2026-09-19, ask_user `0ecdc2ab`)**:
> - **D21 = 会员档**: 「跟进推荐」(复购预测) 归会员 → 会员 = AI 区整块
> - **D22 = 月 30 次 + 累计 360 次** (主人自定义; 比 agent 建议的 2/24 宽松得多 → 见下方风险提示)
> - **D23 = 被推荐用户被提为加盟者后发奖** (触发点 = 该用户成为加盟商那一刻)
> - **D20 = 开工 S0** ✅ (不含支付集成)
>
> ⚠️ **D22 风险提示 (agent 保留意见)**: 封顶越宽, "以发展人员数量计酬"的**外观**越接近《禁止传销条例》
> 关注点。仍守住"只送服务权益 + 仅一层 + 与层级无关"三条时风险可控, 但**律师复核这一条尤其要看**。
> 实现上做成可配常量 (`REFERRAL_MONTHLY_CAP` / `REFERRAL_TOTAL_CAP`), 随时能收紧。
>
> **待主人确认的小口径**: D23 的"发奖"指**推荐人**的 15 天 (被推荐人自己的 15 天建议仍在其注册+验证后发放,
> 否则新用户填了码却看不到任何反馈)。实现里两个触发点都做成可配: `referrer: on_franchisee` / `referee: on_verify`。
>
...
| 通道 | 门槛 | 建议 |
|---|---|---|
| **个人微信收款码 + 人工核销** | 无集成 | ✅ **已落地 (S0.5, 主人 2026-09-19 拍)**: 内测先这么收 —— 用户扫码付款 + App 内「我已支付」+ 管理员 App 内核销 (详见 ADR-0012 §7.1) |
| 个体户收款码 / 对公转账 + 人工核销 | 无集成 | S1 起用 (个体户实名收款更规范) |
...
| **D21** | 复购预测 (零成本 AI 区功能) 归哪档 | 会员 / 免费钩子 | ✅ **主人已定: 归会员** (AI 区块整块是会员) | — |
| **D22** | 推荐奖励封顶 | ✅ **主人已定: 月 30 次 + 累计 360 次** | 做成可配常量, 建议随律师意见可收紧 (见上方风险提示) | — |
| **D23** | 推荐发奖条件 | ✅ **主人已定: 被推荐人成为加盟者后发表奖** (推荐人侧) | 被推荐人侧 15 天仍在"注册+验证"后发 (待确认, 两触发点均可配) | — |
