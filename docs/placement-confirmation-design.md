...
> **⚠ 追加 (主人 2026-09-21 拍, ADR-0014)**: 新增 **`kind='promote'` 向上认领上级** —— 「往根部发展」。
> 背景: 客户公司现实里**已有固有加盟树**, app 只是把现公司的树**同步**进来; admin 指定的初始用户
> 大概率只是公司体系里的**中间层** (例如第 10 层) → 老三方确认只能往下长, 上面接不进来。
> 做法: 现根 A 认领现实里的直接上级 U → **U 成新根, A 整棵子树下降一层** (可反复上溯)。
> 确认方 = **双方** (A + U 本人, 与"父节点==设置者→双方"同一条规则, 代码零特例)。
> **往下生长的老三方确认完全不变**。另: 多根支持 (`franchisee.root_id`) 同时落地, 见 ADR-0014。
>
> **⚠ 追加 (主人 2026-09-21 同日第二轮 5 条, ADR-0014 §3.6)** —— promote 的三处细化:
> 1. **「上层」= 点位父** (≠ 推荐码提供人): 上层格按 `placement_path 去尾段 + 同 root_id` 认, 不看 `referrer_id`
> 2. **上层一旦有人不能撤换**: 全仓无换上层入口; 确需调整联系系统管理员协商处理
> 3. **可认领「已在 app 里是节点」的人** (`upline_fid`, migration 0018): 前提是他的**一层 2 个点位有空位**
>    → 复用它现有节点, **两棵树在此合并** (不建副本); 他没有可登录账号 → 拒 (确认要他本人点)
> 4. **我在上级的 A线/B线 由上级自己挑** (发起时免传 `side`; `decide` 接第 5 参 `side`):
>    两条都空必须给; 只剩一条自动落; 传了已有人的 → 400
> 5. 「谁有权去拉一个上级」的唯一判据: **`placement_path === ''`** (上层虚位) ——
>    admin 建根拉起的 / 被自己下线认领进来的 → 可认领; 被其他加盟用户拉进来的 → 上面已有人 → 无权限
>
...
3. **三方确认**：设置者本人 + 新加盟商本人 + 新位置的上一个节点加盟商（父节点 == 设置者时 → 双方）
   - 追加 (2026-09-21): **向上认领上级** (`kind='promote'`) 走**双方** —— app 里根本没有"上上层"那个人,
     也就没人能当老三方 (锚点 = 现根自己, 同"父==设置者")
