...
**版本**: v0.1.6
**生效日期**: 2026-09-05
**修订日期**: 2026-09-23 (v0.1.6 — 核心定位四要素: 客户管理为核心 + 三大动作 + 跟进指引 + 付费/加盟转化, 详见 §1.1 + §1.4 + ADR-0018)
...
> **暖客宝** —— 大健康行业的**客户管理系统**。**核心 = 客户管理**(记录 / 管理 / 分析),**AI 助手**给出明确的**跟进指引**,让销售员在对的时机做对的事 —— **提高付费率与加盟率**。
...
### 1.4 核心定位四要素 (Positioning — What & Why)

> **2026-09-23 主人核实定位** (ADR-0018): 「本应用的核心功能是客户管理。协助用户记录、管理、分析客户信息,给用户明确的行动(跟进)指引,进而提高用户付费率、加盟率。」

| 维度 | 口径 | 当前落地 |
|---|---|---|
| **核心功能** | **客户管理** | `flutter_app/lib/modules/customer/` 主战场;`src/lib/db/queries/customer-scope.ts` 单一真相源 |
| **三大动作** | **记录 + 管理 + 分析** | 记录 = 客户档案 + 养生记录结构化表单(原则 4);管理 = 归属 / 跟进任务 / 联系人;**分析 = 跟进指引**(现状弱, 后期 AI 域补强) |
| **行动输出** | **明确的跟进指引**(AI 助手承载) | 当前 = 跟进任务(`modules/follow_up/`)+ Phase 2 AI Copilot 跟进建议 / 话术 |
| **终极目的** | **付费率 + 加盟率** | 当前 = 加盟树结构(ADR-0006)+ 用户身份模型(ADR-0015/0016);**付费率**当前无金额字段(§3.6 红线),仅通过复购 / 转化代理指标 |

**对开发的影响**(任何新功能提案必答以下四问):

1. 它服务于 **记录 / 管理 / 分析** 哪一个动作?
2. 它产出**跟进指引**吗? 产出的指引可执行吗?(AI 能力**不为炫技**, 为可执行)
3. 它最终能 **提高付费率 / 加盟率** 吗? 不能 → 重做或砍掉
4. 它是否引入了**金额 / 计酬**机制?(§3.6 红线 = 否)

**角色重定义**(旧描述 vs 新定位):

- "**养生记录**": 旧定位下是并列三件套之一;新定位下是**实现手段**(服务于"分析"+"跟进指引"),不是核心
- "**AI 助手**": 旧定位下是并列三件套之一;新定位下是**跟进指引的载体**,产出形态 = 行动指引
- "**加盟体系**": 旧定位下是业务结构(ADR-0006);新定位下是**付费 / 加盟转化的载体**,服务于终极目的
...
| v0.1.5 | 2026-09-22 | 已被 v0.1.6 追加 | web admin 解冻 (结束 mobile-only) + 用量域: §4.4 重写 (freeze-keep → 活跃, flutter-only-sync → 双线同步, 加 §4.4.5 用量域红线) / §4.2 加用量域行; 配套 ADR-0017 |
| v0.1.6 | 2026-09-23 | **生效** | **核心定位四要素** (客户管理为核心 + 记录/管理/分析三大动作 + AI 跟进指引为行动输出 + 付费率/加盟率为终极目的): §1.1 一句话定义改写 / §1.4 **新增** (四要素表 + "对开发的影响"四问 + "角色重定义"子段); 下游同步 AGENTS §1 + README 顶部与"这个项目是干嘛的"; 配套 **ADR-0018** |
