...
## [Unreleased] — 加盟状态卡: 胶囊移到标题行, 删类型徽章 (2026-09-25)

主人 2026-09-25: 「区块的右上角图标可以删掉。把普通/加盟键移动到区块右上角
（与标题'加盟状态'同一行）」。

- 删掉该卡右上角的**类型徽章** (`FranchiseChip`) —— 胶囊的选中态本身就是状态显示,
  徽章是重复的第二个编码。
- **普通/加盟 胶囊移到标题行** (右侧, 与「加盟状态」同一行); 已是加盟商时胶囊
  置灰显示"加盟" (退出仍走「解除加盟」, 不能在这里切)。
- 测试: ⑩ 例补断言 (胶囊与标题同一行 + 在标题右侧 + 页面里只剩头部那一个类型徽章)。
...
- 区块标题「客户类型」→「**加盟状态**」(类型徽章保留)。
- 胶囊两档: `普通 / 🌱 种子` → **`普通 / 加盟`**;
  **选「加盟」直接进加盟落位流程** (需三方确认: 你 + 她本人 + 目标上级) ——
  原来那个整行「发展为加盟商」大按钮**删除**, 入口收进这颗胶囊。
- 已是加盟商 → 不显示胶囊 (改一句说明: 状态由加盟关系决定, 退出要走「解除加盟」)。
- 顺手删掉不再使用的 `_setCustomerSeed` (种子不再由这张卡切换 —— 种子的口径在
  「客户来源」轴, 见另一批 D3/D5 改动)。
- 测试: 更新「管理 Tab 内容还在」断言 (客户类型 → 加盟状态) + 新增 1 例
  (普通/加盟两档 / 无 🌱 种子 / 无「发展为加盟商」整行按钮)。

验证: `flutter analyze` 0 issue; 相关 9 个测试文件 **79/79** 全绿。

## [Unreleased] — 「我的」页删掉重复的「换头像」按钮 (2026-09-25)

主人 2026-09-25: 「删掉头像正面的『换头像』标签，因为点头像就能换头像」。

- `profile_page.dart` 头部卡: 删 `TextButton.icon('换头像')` (头像本身可点 + 相机角标已足够提示);
  头像与姓名间距 `s6` → `s10`
...
- 该模块**只在新建客户时**出现 (把刚加好友的潜在客户直接标成种子);
  **编辑时移除** —— 客户类型在客户详情「管理」Tab 的「客户类型」卡里改
  (普通 / 🌱 种子), 表单里再来一个是重复模块。
- 测试: 新增 2 例 (新建有 / 编辑没有; 同时守住编辑模式仍有「保存修改」、
  推荐码仍只在新建显示)。

## [Unreleased] — 编辑客户页紧凑排版 + 吸底保存 (2026-09-25)

主人 2026-09-25: 「编辑客户页，排版要更紧凑，例如姓名、性别、电话完全可以并排到同一行，
其他条目你根据情况也做排版优化。保存键固定置底」。

- **姓名 / 性别 / 手机号 一行** (4:2:4 分栏, 窄屏 390 也不溢出);
  性别从「三个大按钮独占一行」改成**紧凑下拉** (这栏本来就只有一个值)。
- **历法收进「生日」标题行** (阳历/农历 SegmentedButton) —— 省掉一整行。
- **备注** 3 行 → 2 行; **种子客户开关** 内边距压紧 (s16/s8 → s12/s4)。
- **保存键吸底** (`bottomNavigationBar` + SafeArea) —— 不用滑到底才能保存。
- 测试: 新增 `customer_form_compact_test.dart` 3 例
  (一行三栏 + 窄屏不溢出 / 历法同标题行 / 吸底不随滚动)。

## [Unreleased] — 编辑入口从顶栏移到基础信息卡 (2026-09-24)

主人 2026-09-24: 「客户详情页右上角的编辑图标需要删除。不要全局编辑页。
在客户基础信息卡片右上角显示编辑图标就行」。

- **删掉 AppBar 的编辑图标** —— 它在「记录/分析/管理」三个 Tab 都可见,
  给人"全局编辑"的感觉。
- **基础信息卡 (管理 Tab) 右上角加「编辑档案」图标** (Stack 浮在角落, 不挤占
  原来的居中布局); 流程不变, 仍走 `/customers/:id/edit` 表单页。
- 测试: 新增 1 例 —— 顶栏无「编辑」入口; 「编辑档案」只在管理 Tab 出现
  (记录 Tab 看不到 = 确实是"卡片级"入口, 不是全局的)。

## [Unreleased] — 客户详情「管理」Tab 9 项改进 (2026-09-24)

主人 2026-09-24: 「执行你的全部9条建议」。

### P0 安全 / 信任

1. **记录表单顶部加健康警示条** —— 过敏史 / 既往病史 (有才显示, 只读, 拿不到档案就静默不显示)。
   原来「添加养生记录」只拿 customerId 取字典, **完全不显示健康信息**; 现在做项目前一定看得到。
2. **「最近改动」审计卡** (管理 Tab 底部) —— 归属转移 / 合并 / 绑定 / 归档, 谁改的、什么时候改的。
   - 新 `GET /api/customers/:id/audit` (RBAC 与详情**同一把尺**: 看不见的客户 404)
   - 新 `src/lib/db/queries/audit.ts` (⚠ 查询**必须带 `table_name`**, `record_id` 跨表不唯一)
   - **只返回变更列名, 不返回列值** (列值可能是密文/手机号/病史) → UI 映射中文标签;
     `updated_at`/`created_at` 噪音列不展示

### P1 信息 / 效率

3. **档案信息补全**: 备注 + 最后修改时间 (之前管理 Tab 只显示到健康标签)
4. **手机号一键复制** (复制后 SnackBar 提示)
5. **生日提醒档位就地可改**: 提前 7 天 / 3 天 / 当天 / 不提醒 (PATCH 后刷新详情)
6. **app 身份卡显示「她的邀请码」+ 复制** —— 后端 `CustomerView` 新增 `accountReferralCode`
   (取数走 `user.customer_id` 连接 + `referral_code`; 没账号 = null)

### P2 结构 / 视觉

7. **管理 Tab 三节分组**: 档案 / 关系与身份 / 最近改动 (危险操作仍在最后, 视觉隔离)
8. **档案卡压紧**: 性别/年龄/建档 三个 chip → 一行 `女 · 42 岁 · 建档 2026-09-01`;
   「点头像可以换…」说明 → 相机角标 tooltip
9. **健康提示独立成卡** (过敏 = 暖橙 / 病史 = 红 / 没填时一条中性轻提示)

### 验证

- 后端: `npx vitest run` (test 库) **651/651** 全绿
  (新增 `tests/customer-audit.test.ts` 3 例: 建档/改名审计 + **table_name 隔离** + limit 倒序)
- 前端: `flutter test` 全量 **487/487** 全绿
  (新增审计卡 4 例 / 健康警示条 2 例 / 管理 Tab 分组+邀请码 2 例);
  `flutter analyze` 0 issue; 真浏览器 13/13; `check-ui-tokens --strict` 通过
...
- 快捷 chips: `7 / 14 / 30 天后` → **`明天 / 3 天后 / 7 天后 / 10 天后`**
  (明天 = 1 天, 文案单独处理; 回访节奏基本落在这四档)。
- 测试: 新增 1 例 —— 四个档位标签正确 + 旧档位(14/30)已删 + 点「明天」→
  日期按钮显示明天且 chip 选中。养生相关 7 个测试文件 **32/32** 全绿。
- ⚠ 说明: 本批代码被 auto-snapshot hook 抢先提交进 `[SNAPSHOT]` 提交
  (同一提交里还含**另一个 session** 的在制品), 并发环境下**未做历史重排**。

## [Unreleased] — 照片区块收口: 删标题行按钮 + 数量贴标题右 (2026-09-24)

主人 2026-09-24: 「既然已经可以直接点缩略图位置选择拍照/相册上传，区块右上角的拍照和
相册按键可以删除了。已上传数量移动到标题（部位照片）右侧」。

- **删除**标题行右侧的「拍照 / 相册」两个按钮 —— 加照片统一走**点空槽位**
  (空槽位上有相机图标 → 弹「拍照 / 从相册选 (可多选)」)。
- 「已上传 N / 5」从副标题**移到标题右侧** (`AppSectionHeader.action`)。
- 测试: ③ 改为「数量在标题右侧 + 同一行 + 两个按钮已删」; 新增 ④「点空槽位弹选择层」;
  `flutter test` 全量 **411/411** 全绿 (--concurrency=1; 并发跑过一次 900s 超时,
  是机器被其它 session 占满导致的假挂, 单跑全部通过)。

## [Unreleased] — 常用短语不再重复写入 (2026-09-24)

主人 2026-09-24: 「操作过程和客户反馈要限制同一个常用短语被多次点选。当前情况是
同一个常用短语被多次点击后被多次写入同样的内容了」。

- 短语 chip 分两种状态: **未加** (可点, 点击追加) / **已加** (打勾 + 置灰不可点)。
  判定用 `文本.contains(短语)` —— 手动把这句话打进文本框也算"已加"。
- 追加时再做一次**二次判定** (防"文本刚被手打进, 但 chip 还没重建"的竞态):
  已含则只触发一次重建把 chip 刷成已加态, **不重复追加**。
- 测试: 新增 2 例 (连点同一 chip 不重复 + 手打后点 chip 也不重复), 全量 **410/410**。

## [Unreleased] — 添加养生记录页: 13 项改进批量落地 (2026-09-24)

主人 2026-09-24: 「做你建议的1到12条+14条（除了第13条AI引导外的全部）」。
(AI 引导 = 第 13 条, 按主人要求**不做** —— 会员额度不该自动烧。)

### P0 数据正确性 / 防丢

1. **修 bug: 编辑记录会把日期改成今天** —— `_submit` 原来一律发 `serviceDate = 今天`,
   编辑 9-14 的老记录会把它挪到今天 (时间线/趋势/复购周期全跟着错)。
   现在新增 `_serviceDate` 状态, 编辑时从原记录载入。
2. **记录日期可选 (补录)** —— 新增「记录日期」区块 (默认今天, 可往前选 2 年)。
3. **填一半离开会拦一下** —— `PopScope` + 脏标记: 有未保存改动时确认
   「继续填写 / 离开」(草稿已自动暂存, 所以是提醒不是"要丢了")。
4. **下次建议日期可清除** —— 选中后右侧出现 ✕。

### P1 提效

5. **下次建议日期快捷档位** —— 「7 天后 / 14 天后 / 30 天后」chips (点了直接设),
   比开日期选择器快 3 步。
6. **操作过程 / 客户反馈: 常用短语 chips** —— 点一下追加 (` · ` 分隔), 保留自由输入;
   中年用户打字慢, 这两栏是全页最慢的一段。
7. **保存吸底** —— 长表单不再需要滑到底才能保存 (`bottomNavigationBar`)。
8. **同一天重复记录提醒** —— 该客户这天已有记录 → 保存前问一句「继续保存 / 取消」。
   (查询失败不拦: 宁可少一次提醒, 不能挡住保存。)

### P2 交互

9. **空照片槽位可点** —— 点它弹「拍照 / 从相册选 (可多选)」。
10. **相册多选** —— `pickMultiImage`, 一次补足到上限, 超出部分提示丢弃。
11. **服务项目: 下拉 → 底部弹层 + 搜索** —— 项目多了下拉又长又难找。
12. **身体部位: 已选置顶 + 未选超 8 个折叠** —— 「还有 N 个 / 收起」。

### 边界

14. **草稿自动暂存** (shared_preferences) —— 改动后 600ms 暂存, 重进同客户的表单
    优先恢复草稿 (超 7 天视为陈旧清掉); 保存成功 / 清空重填会清掉草稿。
    平台不支持时静默关闭 (绝不能因为草稿把表单打不开)。

### 验证

- `flutter analyze` 0 issue; `flutter test` 全量 **408/408** 全绿
  (新增 `wellness_record_save_test` 3 例: 新建发今天 / **编辑保留原日期 (bug 回归)** /
  部位已选置顶+折叠; 既有养生表单 6 文件 25 例同步更新)
- `check-ui-tokens --strict` 通过; 真浏览器 13/13。

## [Unreleased] — 养生表单: 日期/照片收进标题行 + 照片 5 张等宽一行 (2026-09-24)

主人 2026-09-24: 「下次建议日期区块，选择日期按键太大，可以收到标题（下次建议日期）
同一行。拍照和相册这两个按键也可以收到与标题（部位照片）同一行。上传照片最多5张，
照片等宽排在同一行」。

- **下次建议日期**: 独占一行 48pt 的整宽按钮 → **标题行右侧的紧凑按钮** (44pt,
  未选显示「选择」/ 已选显示日期), 用契约组件 `AppSectionHeader.action`。
- **部位照片**: 「拍照 / 相册」两个半行 64pt 大按钮 → **标题行右侧两个紧凑按钮**;
  「已上传 N / 5」挪到副标题 (原来单独占一行)。
- **照片条**: 固定 **5 个等宽槽位、永远一行** (空槽位显示浅底占位) ——
  张数变化时缩略图不会忽大忽小, 空槽位也顺带表达「还能加几张」;
  默认 `maxPhotos: 6 → 5`。
- 缩略图改为填满槽位 (`StackFit.expand`), 不再写死 96px 固定尺寸。

验证: `flutter analyze` 0 issue; `flutter test` 全量 **405/405** 全绿
(新增 3 例: 默认 5 张 / 5 槽位等宽同行 / 按钮与标题同行); 真浏览器 13/13;
