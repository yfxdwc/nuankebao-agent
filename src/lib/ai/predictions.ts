...
import { customer, wellnessRecord, followUpTask } from "@/lib/db/schema";
import { eq, and, isNull, desc, sql } from "drizzle-orm";
import { aiComplete } from "./client";
import { buildEffectAnalysisPrompt } from "./prompts";
...
export async function generateEffectAnalysis(
  customerId: bigint
): Promise<EffectAnalysisResult | null> {
  // 1. 取客户
  const [cust] = await db
    .select()
    .from(customer)
  ...
  // 2. 取所有养生记录 (按时间升序, 给 AI 看趋势)
  const records = await db
    .select()
    .from(wellnessRecord)
    .where(eq(wellnessRecord.customerId, customerId))
    .orderBy(wellnessRecord.serviceDate);
  ...
  const { listAllDictionaries } = await import("../db/queries/dictionary");
  const dict = await listAllDictionaries();
  ...
  const viewRecords = records.map((r) => ({
    ...
    preCondition: r.preConditionEncrypted
      ? safeJsonParse(decryptField(r.preConditionEncrypted))
      : {},
    postCondition: r.postConditionEncrypted
      ? safeJsonParse(decryptField(r.postConditionEncrypted))
      : {},
  ...
  // 4. 简单趋势分析 (对比首次 vs 最近)
  const trend = computeTrend(viewRecords);

  // 5. 调 AI
  const { system, prompt } = buildEffectAnalysisPrompt({
    customerName: cust.name,
    records: viewRecords,
  });
  ...
  return {
    ...
    dateRange: {
      from: String(records[0].serviceDate),
      to: String(records[records.length - 1].serviceDate),
    },
...
/**
 * 简单趋势: 对比首次和最近一次的 pain_level
 * - 降低 → improving
 * - 相同 → stable
 * - 升高 → worsening
 */
function computeTrend(
  records: Array<{ preCondition: Record<string, unknown>; postCondition: Record<string, unknown> }>
): "improving" | "stable" | "worsening" | "unknown" {
  if (records.length < 2) return "unknown";

  const first = records[0].preCondition.pain_level as number | undefined;
  const last = records[records.length - 1].preCondition.pain_level as number | undefined;
